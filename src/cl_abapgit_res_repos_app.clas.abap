CLASS cl_abapgit_res_repos_app DEFINITION
  PUBLIC
...
CLASS cl_abapgit_res_repos_app IMPLEMENTATION.
  ...
  METHOD register_resources.
    ...
    registry->register_discoverable_resource(
        url             = '/repos'
        accepted_types  = VALUE #( ( |'application/abapgit.adt.repo.v1+xml'| )
                                   ( |'application/abapgit.adt.repo.v2+xml'| )
                                   ( |'application/abapgit.adt.repo.v3+xml'| )
                                   ( |'application/abapgit.adt.repo.v4+xml'| ) )
                                   "cl_abapgit_res_repos=>co_content_type_repo_v1/2/3/4
        handler_class   = 'CL_ABAPGIT_RES_REPOS'
    ...
    registry->register_discoverable_resource(
        ...
        accepted_types  = VALUE #( ( |application/abapgit.adt.repo.info.ext.request.v1+xml| )
                                   ( |application/abapgit.adt.repo.info.ext.request.v2+xml| ) )
                                   "cl_abapgit_res_repo_info_ext=>co_content_type_request_v1/2
        handler_class   = 'CL_ABAPGIT_RES_REPO_INFO_EXT'
    ...
    registry->register_resource(
        template        = '/repos/{key}'
        handler_class   = 'CL_ABAPGIT_RES_REPO' ).

    registry->register_resource(
        template        = '/repos/{key}/pull'
        handler_class   = 'CL_ABAPGIT_RES_REPO_PULL' ).
    ...
    registry->register_resource(
        ...
        handler_class   = 'CL_ABAPGIT_RES_REPO_OBJ_LOG' ).

    "stage
    registry->register_resource(
        template        = '/repos/{key}/stage'
        handler_class   = 'CL_ABAPGIT_RES_REPO_STAGE' ).

    "push
    registry->register_resource(
        template        = '/repos/{key}/push'
        handler_class   = 'CL_ABAPGIT_RES_REPO_PUSH' ).

    "repository checks
    registry->register_resource(
        template        = '/repos/{key}/checks'
        handler_class   = 'CL_ABAPGIT_RES_REPO_CHECKS' ).

    "files
    registry->register_resource(
        template        = '/repos/{key}/files'
        handler_class   = 'CL_ABAPGIT_RES_REPO_FILES' ).
